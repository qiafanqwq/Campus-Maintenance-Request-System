...
const handleLogin = async () => {
  ...
  try {
    ...
    if (response.code === 1) {
      ...
      // 根据用户权限跳转
      // 1-超级管理员、2-维修人员、0-普通用户、3-普通管理员
      if (userData.authorityId === 1  ||  userData.authorityId === 3) {
        await router.push('/adminDashboard/RepairOrderList')
      } else if (userData.authorityId === 2) {
        await router.push('/repairmanDashboard/RepairRequests')
      } else if (userData.authorityId === 0 ) {
